...
const PredictionMarketChart: React.FC = () => {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (
                <Box bg="white" p={2} border="1px solid #ccc" borderRadius="md" boxShadow="md">
                    <Text fontWeight="bold">{label}</Text>
                    <Text color="#8884d8">Yes: {(payload[0].value * 100).toFixed(1)}%</Text>
                    <Text color="#82ca9d">No: {((1 - payload[0].value) * 100).toFixed(1)}%</Text>
                </Box>
            )
        }
        return null
